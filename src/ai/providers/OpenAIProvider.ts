...
export class OpenAIProvider extends BaseAIProvider {
  ...
  protected async callAPI(
    ...
    if (!this.config.apiKey || this.config.apiKey.trim() === '') {
      throw new OpenAIAPIError('Clé API non configurée', 'AUTH');
    }
    ...
    try {
      ...
      if (!data.choices || data.choices.length === 0) {
        throw new OpenAIAPIError('Réponse API invalide', 'INVALID_REQUEST');
      }
      ...
      if (error instanceof DOMException && error.name === 'AbortError') {
        throw new OpenAIAPIError('Délai d\'attente dépassé', 'TIMEOUT');
      }

      // Handle network errors
      if (error instanceof TypeError && error.message.includes('fetch')) {
        throw new OpenAIAPIError('Impossible de contacter l\'API OpenAI', 'NETWORK');
      }
  ...
  private async handleErrorResponse(response: Response): Promise<never> {
    ...
    switch (response.status) {
      case 401:
        throw new OpenAIAPIError(errorMessage, 'AUTH', response.status);
      case 429: {
        const retryAfter = parseInt(response.headers.get('retry-after') || '60', 10);
        throw new OpenAIAPIError(errorMessage, 'RATE_LIMIT', response.status, retryAfter);
      }
      case 400:
        // Check for context length error
        if (errorData?.error?.code === 'context_length_exceeded') {
          throw new OpenAIAPIError(errorMessage, 'CONTEXT_LENGTH', response.status);
        }
        throw new OpenAIAPIError(errorMessage, 'INVALID_REQUEST', response.status);
      case 500:
      case 502:
      case 503:
      case 504:
        throw new OpenAIAPIError(errorMessage, 'SERVER', response.status);
      default:
        throw new OpenAIAPIError(errorMessage, 'UNKNOWN', response.status);
    }
  }
}
...
export class OpenAIAPIError extends Error {
  ...
  get userMessage(): string {
    switch (this.code) {
      ...
      case 'SERVER':
        return 'Erreur serveur OpenAI. Réessayez dans quelques instants.';
      default:
